...
class SymlinkAdapter implements AdapterInterface
{
    ...
    public function __construct(string $rootPath, string $filePath)
    {
		...
		// do not use "realpath" so we can use symlinks for the images and files directories
        // $this->rootPath = Path::clean(realpath($rootPath), '/');
	    $this->rootPath = Path::clean(rtrim($rootPath, '\/'), '/');
	 ...
	 *
	 * @param null $date The date to create a Date from
	 *
	 * @return  Date
	 *
	 * @throws Exception
    ...
    public function copy(string $sourcePath, string $destinationPath, bool $force = false): string
    {
	    ...
	    return str_replace($this->rootPath, '', $destinationPath);
    }
